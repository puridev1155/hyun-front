                  ...
                  <div>
                    ...
                    <div class="mt-4">
                      <div class="mt-4 grid grid-cols-1 gap-y-6 sm:grid-cols-2 sm:gap-x-4">
                        ...
                        <input type="file" name="file" @change="handleFile" :v-model="imageFile"
                          ...
                          <div class="mt-6">
                            <button @click="cancelFile" v-if="imageUrl || form.image || form.original_url"
                                  ...
                                  type="button">삭제
                            </button><img v-if="imageUrl ? imageUrl : form.original_url" :src="imageUrl ? imageUrl : form.original_url" alt="Preview" class="w-10 ">
                            </div>                  
                        </div>  
                    </div>
                    <div>
                    ...
                    <div v-if="form.price_agree || form.age_agree || form.ticket_agree" class="mt-4 grid grid-cols-1 gap-y-6 sm:grid-cols-2 sm:gap-x-4">
                      <div v-if="form.price_agree">
                          <label for="visibility" class="block text-sm font-medium text-gray-700 mb-1">가격을 입력하세요</label>
                          <div class="mt-1">
                            <input type="text" name="price" v-model="form.memo_price" placeholder="3000"
                            class="block w-full rounded-md border-gray-300 bg-white py-3 px-4 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                          </div>
                        </div>
                        <div v-if="form.price_agree">
                        ...
                        <div v-if="form.price_agree" v-for="(row, index) in form.eprices" :key="index">
                        ...
                        <div  v-if="form.age_agree">
                          <label for="visibility" class="block text-sm font-medium text-gray-700 mb-1">Age Insert (나이 한도를 입력하세요)</label>
                          <div class="mt-1">
                            <input type="number" name="age" v-model="form.age_limit" placeholder="20" maxlength="2"
                              class="block w-full rounded-md border-gray-300 bg-white py-3 px-4 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                          </div>
                        </div>
                        <div  v-if="form.ticket_agree">
                          <label for="visibility" class="block text-sm font-medium text-gray-700 mb-1">Ticket Insert (숫자만 입력하세요)</label>
                          <div class="mt-1">
...
form.value = await store.crudApi(null, null, `/admin/posts/${route.query.id}`, 'get'); //Parameter (body, params.  postUrl, methodType)

//Static Top Nav 
const navs = [
  { name: 'Ticket', href: '#', current: false },
  { name: 'Ticket Table', href: '#', current: false },
...
const cancelFile = (event) => {
  imageUrl.value = null;
  form.value.original_url = null;
}
//File Handling End

const updatePost = () => {
    //console.log(form.value.eprices)

    //console.log(form.value.eprices, 'sadfjsajflsajfkadjflas')
  Swal.fire({
    title: '진행하시겠습니까?',
    icon: 'warning',
    ...
    if (result.isConfirmed) {
      ...
      form.value.memo_price = form.value.price_agree ? form.value.memo_price : 0; //disagree시에 0으로 리셋
      form.value.age_limit = form.value.age_agree ? form.value.age_limit : 0;
      form.value.ticket_count = form.value.ticket_agree ? form.value.ticket_count : 0;
      form.value.start_date = form.value.time_agree ? form.value.start_date : null;
      form.value.end_date = form.value.time_agree ? form.value.end_date : null;
      form.value.price_agree = form.value.price_agree ? 1 : 0;
      form.value.sns_agree = form.value.sns_agree ? 1 : 0;
      form.value.eprices = JSON.stringify(form.value.eprices); //eprice array convert to JSON data
      ...
      store.crudApi(formData, form.value, `/admin/posts/${route.query.id}`, 'post'); //Parameter (body, params.  postUrl, methodType)  
