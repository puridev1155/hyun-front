            ...
            <table class=" min-w-full divide-y">
              <thead class="bg-gray-100">
                <tr class="text-gray">
                  ...
                  <th scope="col" class="px-3 py-3.5 text-sm font-semibold text-center">TIKTOK</th>
                  <th scope="col" class="px-3 py-3.5 text-sm font-semibold text-center">GROUP</th>
                  <th scope="col" class="px-3 py-3.5 text-sm font-semibold text-center">QUANTITY</th>
              ...
              <tbody class="divide-y divide-gray-200 bg-white">
                <tr v-for="(row,index) in store.list" :key="index">
                  ...
                  <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 text-center">
                    {{ number(index) }}</td>
                    <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 text-center">
                    {{  categoryList.filter(obj => obj.id === row.category_id)[0].category_name }}</td>
                    <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 text-center">
                    ...
                    <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 text-center">
                      <nuxt-link :to="`https://www.instagram.com/${row.instagram}`"><img v-if="row.instagram2 ? row.instagram2 : row.instagram" class="font-bold w-8 h-8 mx-auto" src="@/assets/images/instagram.jpg" /></nuxt-link></td>
                    <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 text-center">
                      <nuxt-link :to="`https://www.tiktok.com/@${row.tiktok}`"><img v-if="row.tiktok" class="font-bold w-8 h-8 mx-auto" src="@/assets/images/tiktok.png" /></nuxt-link></td>
                    <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 text-center">  
                      {{ row.participant }}
                    </td>
                    <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 text-center">  
...
import { participantsInfo, dataCrud } from '../../stores/index'
...
const stats = [
  ...
  { name: 'DAILY BUYERS', value: '', unit: 'users' },
  { name: 'WEEKLY BUYERS', value: '150', unit: 'users' },
  { name: 'MONTLY BUYERS', value: '700', unit: 'users' },
]
...
function deleteData(id) {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
     ...
     await $fetcher(useRuntimeConfig().public.backendUrl + `/api/admin/participants/${id}`, {
        method: "delete",
        credentials: 'include',
      });
