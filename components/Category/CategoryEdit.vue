                ...
                <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                  <div class="sm:flex sm:items-start">
                    <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                      <DialogTitle as="h3" class="text-base font-semibold leading-6 text-gray-900">카테고리 수정
                      </DialogTitle>
                  ...
                  <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6 justify-center mt-3">
                    ...
                    <button type="button"
                      ...
                      @click="categoryEdit" ref="cancelButtonRef">수정</button>
                  </div>
                </div>
...
async function categoryList()  {
  const response = await axios.get(`http://ec2-13-209-67-110.ap-northeast-2.compute.amazonaws.com/api/category/${props.postId}`)
  name.value = response.data.category_name
}
...
function editCategory() {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      axios.post(`http://ec2-13-209-67-110.ap-northeast-2.compute.amazonaws.com/api/category/${props.postId}`, form)
        .then((response) => {
