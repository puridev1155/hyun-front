<template>
  <nav class="flex items-center justify-center border-t border-gray-200 px-4 sm:px-0">
    <div class=" -mt-px flex">
      ...
      <!-- 페이지 숫자-->
      <ul>
        <li class="float-left"  v-for="items in pageList" :key="items">
...
const currentPageGroup = computed(() => Math.ceil(store.page / pageDisplayCount.value));
  ...
  const nextPage = () =>  {
    ...
    store.page++
    // router.push({ path: `/admin/user/${store.page},${store.search},${store.startdate},${store.enddate}`, query: { page: store.page, state:store.state, election: store.election, district: store.district } })
